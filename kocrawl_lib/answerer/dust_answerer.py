...
class DustAnswerer(BaseAnswerer):
    ...
    def morning_afternoon_form(self, location: str, date: str, result: dict, josa: list) -> str:
        ...
        msg += '<br>{date} 오전 미세먼지 상태{j0} {morning_fine_dust} 오후 상태{j1} {afternoon_fine_dust} ' \
               '<br>오전 초미세먼지 상태{j2} {morning_ultra_dust} 오후 상태{j3} {afternoon_ultra_dust} ' \
               '<br>오전 대기중 오존 상태{j4} {morning_ozon} 오후 상태{j5} {afternoon_ozon} ' \
